...
fn discard_binding() {
    ...
    assert!(matches!(
        d.kind,
        DestructureKind::Binding(Binding::Discarded)
    ));
}
...
fn single_element_array() {
    ...
    match d.kind {
        DestructureKind::Array { elements, rest } => {
            ...
            assert!(
                matches!(&elements[0].kind, DestructureKind::Binding(Binding::Named(n)) if n == "a")
            );
            assert!(rest.is_none());
        }
        other => panic!("expected Array, got {other:?}"),
...
fn multiple_elements() {
    ...
    match d.kind {
        DestructureKind::Array { elements, rest } => {
            ...
            assert!(
                matches!(&elements[1].kind, DestructureKind::Binding(Binding::Named(n)) if n == "b")
            );
            assert!(
                matches!(&elements[2].kind, DestructureKind::Binding(Binding::Named(n)) if n == "c")
            );
            assert!(rest.is_none());
        }
        other => panic!("expected Array, got {other:?}"),
        ...
        DestructureKind::Array { elements, rest } => {
            ...
            match &elements[1].kind {
                DestructureKind::Array {
                    ...
                    assert!(
                        matches!(&inner[0].kind, DestructureKind::Binding(Binding::Named(n)) if n == "b")
                    );
                    assert!(
                        matches!(&inner[1].kind, DestructureKind::Binding(Binding::Named(n)) if n == "c")
                    );
        ...
        DestructureKind::Array { elements, .. } => {
            ...
            match &elements[1].kind {
                DestructureKind::Array { elements: mid, .. } => {
                    assert_eq!(mid.len(), 3);
                    assert!(
                        matches!(&mid[0].kind, DestructureKind::Binding(Binding::Named(n)) if n == "b")
                    );
                    assert!(
                        matches!(&mid[1].kind, DestructureKind::Binding(Binding::Named(n)) if n == "c")
                    );
                    match &mid[2].kind {
                        DestructureKind::Array {
                            elements: inner, ..
                            ...
                            assert_eq!(inner.len(), 2);
                            assert!(
                                matches!(&inner[0].kind, DestructureKind::Binding(Binding::Named(n)) if n == "d")
                            );
                            assert!(
                                matches!(&inner[1].kind, DestructureKind::Binding(Binding::Named(n)) if n == "e")
                            );
                        }
...
fn nested_with_rest() {
    ...
    match d.kind {
        DestructureKind::Array { elements, rest } => {
            assert_eq!(elements.len(), 2);
            assert!(
                matches!(&elements[0].kind, DestructureKind::Binding(Binding::Named(n)) if n == "head")
            );
            match &elements[1].kind {
                DestructureKind::Array {
                    elements: inner,
                    ...
                    assert!(
                        matches!(&inner[0].kind, DestructureKind::Binding(Binding::Named(n)) if n == "first")
                    );
        ...
        DestructureKind::Array { elements, .. } => {
            ...
            match &elements[1].kind {
                DestructureKind::Array {
                    ...
                    assert!(
                        matches!(&inner[1].kind, DestructureKind::Binding(Binding::Named(n)) if n == "x")
                    );
                }
                other => panic!("expected nested Array, got {other:?}"),
            }
...
fn map_shorthand() {
    ...
    match d.kind {
        DestructureKind::Map {
            ...
            assert!(
                matches!(&entries[0].destructure.kind, DestructureKind::Binding(Binding::Named(n)) if n == "foo")
            );
            assert_eq!(str_key(&entries[1]), b"bar");
            assert!(
                matches!(&entries[1].destructure.kind, DestructureKind::Binding(Binding::Named(n)) if n == "bar")
            );
            assert!(bind_whole.is_none());
        }
        other => panic!("expected Map, got {other:?}"),
...
fn map_explicit_keys() {
    ...
    match d.kind {
        DestructureKind::Map {
            ...
            assert_eq!(str_key(&entries[0]), b"foo");
            assert!(
                matches!(&entries[0].destructure.kind, DestructureKind::Binding(Binding::Named(n)) if n == "a")
            );
            assert_eq!(str_key(&entries[1]), b"bar");
            assert!(
                matches!(&entries[1].destructure.kind, DestructureKind::Binding(Binding::Named(n)) if n == "b")
            );
            assert!(bind_whole.is_none());
        }
        other => panic!("expected Map, got {other:?}"),
...
fn map_computed_key() {
    ...
    match d.kind {
        DestructureKind::Map {
            ...
            assert!(
                matches!(&entries[0].destructure.kind, DestructureKind::Binding(Binding::Named(n)) if n == "x")
            );
            assert!(bind_whole.is_none());
        }
        other => panic!("expected Map, got {other:?}"),
...
fn map_mixed_keys() {
    ...
    match d.kind {
        DestructureKind::Map { entries, .. } => {
            ...
            assert!(
                matches!(&entries[2].destructure.kind, DestructureKind::Binding(Binding::Named(n)) if n == "c")
            );
        }
        other => panic!("expected Map, got {other:?}"),
    }
        ...
        DestructureKind::Map { entries, .. } => {
            ...
            match &entries[0].destructure.kind {
                DestructureKind::Map { entries: inner, .. } => {
                    ...
                    assert!(
                        matches!(&inner[0].destructure.kind, DestructureKind::Binding(Binding::Named(n)) if n == "val")
                    );
                }
