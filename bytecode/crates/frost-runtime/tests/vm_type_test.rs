...
fn exact_true_on_matching_type() {
    assert_eq!(val(vec![PushInt(1), TypeTest(Ftc::Exact(Ft::Int))]), Value::Bool(true));
}
...
fn numeric_true_on_float_false_on_string() {
    assert_eq!(val(vec![float(1.5), TypeTest(Ftc::Numeric)]), Value::Bool(true));
    let out = eval(vec![Value::from("x")], vec![LoadConst(0), TypeTest(Ftc::Numeric)]).unwrap();
    assert_eq!(out, Value::Bool(false));
}
...
fn primitive_true_on_null_false_on_array() {
    // null is primitive...
    assert_eq!(val(vec![PushNull, TypeTest(Ftc::Primitive)]), Value::Bool(true));
    // ...an array is not.
    let out = eval(
...
fn structured_true_on_map_false_on_int() {
    ...
    assert_eq!(val(vec![PushInt(1), TypeTest(Ftc::Structured)]), Value::Bool(false));
}

#[test]
fn nonnull_false_on_null_true_on_int() {
    assert_eq!(val(vec![PushNull, TypeTest(Ftc::NonNull)]), Value::Bool(false));
    assert_eq!(val(vec![PushInt(1), TypeTest(Ftc::NonNull)]), Value::Bool(true));
}
...
fn consumes_operand_and_pushes_one_bool() {
    ...
    assert_eq!(
        val(vec![PushInt(99), PushInt(1), TypeTest(Ftc::Exact(Ft::Int)), Pop]),
        Value::Int(99)
    );
}
