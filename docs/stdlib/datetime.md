...
All functions which take a timezone default to UTC.
An optional timezone parameter accepts [IANA timezone names](https://en.wikipedia.org/wiki/List_of_tz_database_zones) (e.g., `"America/New_York"`, `"Asia/Tokyo"`).
...
`format(millis, pattern, timezone)`
...
`components(millis)`
`components(millis, timezone)`

Breaks a timestamp into its calendar components.
...
# => {year: 2026, month: 4, day: 20, hour: 6, minute: 55, second: 35, ms: 236, weekday: "Monday"}
```

## `from_components`

`from_components(map)`
`from_components(map, timezone)`

Constructs a timestamp from a components `Map`.
Missing fields default to the epoch: `year` to 1970, `month` and `day` to 1, time fields to 0.
