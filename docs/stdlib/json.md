...
Produces an error on unsupported values.

## `encode_pretty`
`encode_pretty(value, indent)`

Serializes `value` to a pretty-printed JSON string with the given `indent` level (number of spaces per nesting level).
`indent` must be a non-negative `Int`.
Carries the same value restrictions as `encode`.
