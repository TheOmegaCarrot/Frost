...
[[noreturn]] void minus_err(std::string_view lhs_type,
                            std::string_view rhs_type)
{
    op_err("subtract", lhs_type, rhs_type);
}

struct Numeric_minus_Impl
{
...
Value_Ptr Value::subtract(const Value_Ptr& lhs, const Value_Ptr& rhs)
{
    ...
    if (lhs->is_numeric() && rhs->is_numeric())
        return Value::create(std::visit(numeric_minus_impl, lhs_var, rhs_var));

    minus_err(lhs->type_name(), rhs->type_name());
}
