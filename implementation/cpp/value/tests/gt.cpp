...
TEST_CASE("Numeric GT")
{
    ...
    SECTION("Int Identity GT")
    {
        CHECK_FALSE(Value::greater_than(int1, int1)->get<frst::Bool>().value());
    }

    SECTION("Int GT")
    {
        CHECK_FALSE(Value::greater_than(int1, int2)->get<frst::Bool>().value());
        CHECK(Value::greater_than(int3, int1)->get<frst::Bool>().value());
        CHECK_FALSE(Value::greater_than(int1, int3)->get<frst::Bool>().value());
        CHECK_FALSE(Value::greater_than(int3, int4)->get<frst::Bool>().value());
    }

    SECTION("Float Identity GT")
    {
        CHECK_FALSE(Value::greater_than(flt1, flt1)->get<frst::Bool>().value());
    }

    SECTION("Float GT")
    {
        CHECK_FALSE(Value::greater_than(flt1, flt2)->get<frst::Bool>().value());
        CHECK(Value::greater_than(flt3, flt1)->get<frst::Bool>().value());
        CHECK_FALSE(Value::greater_than(flt1, flt3)->get<frst::Bool>().value());
        CHECK_FALSE(Value::greater_than(flt3, flt4)->get<frst::Bool>().value());
    }
...
TEST_CASE("String Lexicographical GT")
{
    auto foo = Value::create("foo"s);
    auto bar = Value::create("bar"s);
    auto foobar = Value::create("foobar"s);

    CHECK(Value::greater_than(foo, bar)->get<frst::Bool>().value());
    CHECK_FALSE(Value::greater_than(bar, foo)->get<frst::Bool>().value());
