...
TEST_CASE("Numeric Division")
{
    ...
    SECTION("FLOAT / FLOAT")
    {
        auto res = Value::divide(flt1, flt2);
    ...
    SECTION("INT / FLOAT")
    {
        auto res = Value::divide(int1, flt1);
        REQUIRE(res->is<frst::Float>());
    ...
    SECTION("FLOAT / INT")
    {
        auto res = Value::divide(flt2, int2);
        REQUIRE(res->is<frst::Float>());
        CHECK(res->get<frst::Float>().value() == 2.17 / 81_f);
    }
}

TEST_CASE("Divide All Permutations")
{
...
#define OP_CHAR /
#define OP_VERB divide
#define OP_METHOD divide
