...
TEST_CASE("cli.parse arity and types")
{
    ...
    {
        CHECK_THROWS_MATCHES(parse->call({}), Frost_User_Error,
    ...
    SECTION("First arg not Array")
    {
        CHECK_THROWS_MATCHES(
            parse->call({Value::create("x"s), Value::create(Value::trusted, Map{})}),
            Frost_User_Error,
...
TEST_CASE("cli.parse spec validation")
{
    ...
    auto call_with_spec = [&](Map spec) {
        return parse->call({args, Value::create(Value::trusted, std::move(spec))});
    };
    ...
    SECTION("Flag with unknown key")
    {
        ...
        Map flags = {{"verbose"_s,
                      Value::create(Value::trusted, std::move(flag_spec))}};
        CHECK_THROWS_MATCHES(
            call_with_spec(
                Map{{"flags"_s,
    ...
    SECTION("Duplicate long name across flags and options")
    {
        ...
        Map opt_spec = {};
        Map options = {{"name"_s,
                        Value::create(Value::trusted, std::move(opt_spec))}};
        CHECK_THROWS_MATCHES(
            call_with_spec(Map{
                {"flags"_s,
                 Value::create(Value::trusted, std::move(flags))},
    ...
    SECTION("Flag short not single character")
    {
        ...
        Map flags = {{"verbose"_s,
                      Value::create(Value::trusted, std::move(flag))}};
        CHECK_THROWS_MATCHES(
            call_with_spec(
                Map{{"flags"_s,
                     Value::create(Value::trusted, std::move(flags))}}),
    ...
    SECTION("Option short not single character")
    {
        ...
        Map options = {{"env"_s,
                        Value::create(Value::trusted, std::move(opt))}};
        CHECK_THROWS_MATCHES(
            call_with_spec(
                Map{{"options"_s,
                     Value::create(Value::trusted, std::move(options))}}),
    ...
    SECTION("positional as non-Bool non-Array")
    {
        CHECK_THROWS_MATCHES(
            call_with_spec(
                Map{{"positional"_s, Value::create(42_f)}}),
            Frost_Recoverable_Error,
...
TEST_CASE("cli.parse successful parsing")
{
    ...
    SECTION("Custom help text is returned verbatim")
    {
        auto result = do_parse(
            {"s.frst"},
            Map{{"help"_s, Value::create("my custom help"s)}});
    ...
    {
        auto result = do_parse(
            {"s.frst"}, Map{{"name"_s, Value::create("deploy"s)}});
    ...
    SECTION("Help contains description")
    {
        auto result = do_parse(
            ...
            Map{{"name"_s, Value::create("deploy"s)},
                {"description"_s,
                 Value::create("Deploy a service"s)}});
    ...
    SECTION("Help contains default marker")
    {
        ...
        auto result = do_parse(
            ...
            Map{{"options"_s,
                 Value::create(Value::trusted, std::move(options))}});
        CHECK(
            get(result, "help")->raw_get<String>().contains("default: 3"));
    }
    ...
    SECTION("Help contains repeatable marker")
    {
        ...
        auto result = do_parse(
            ...
            Map{{"options"_s,
                 Value::create(Value::trusted, std::move(options))}});
        CHECK(
            get(result, "help")->raw_get<String>().contains("repeatable"));
    }
    ...
    SECTION("Help contains positional names")
    {
        Map entry = {{"name"_s, Value::create("service"s)},
                     {"description"_s,
                      Value::create("the thing to deploy"s)}};
        Array pos = {Value::create(Value::trusted, std::move(entry))};
        auto result = do_parse(
            {"s.frst", "web"},
            Map{{"positional"_s, Value::create(std::move(pos))}});
    ...
    SECTION("Help for raw collect positionals")
    {
        auto result = do_parse(
            ...
            Map{{"positional"_s, Value::create(Bool{true})}});
        CHECK(
            get(result, "help")->raw_get<String>().contains("[args...]"));
    }
...
TEST_CASE("cli.parse errors")
{
    ...
    auto expect_error = [&](std::initializer_list<std::string> argv,
                            Map spec, const std::string& substring) {
        CHECK_THROWS_MATCHES(
