...
TEST_CASE("Format_String")
{
    ...
    SECTION("Literal-only string performs no lookups")
    {
        ...
        Format_String node{AST_Node::no_range, "hello world"};
    ...
    SECTION("Single placeholder is substituted using to_internal_string")
    {
        ...
        Format_String node{AST_Node::no_range, "value: ${n}"};
        auto result = node.evaluate(ctx);
        REQUIRE(result->is<frst::String>());
    ...
    SECTION("Multiple placeholders are evaluated in order")
    {
        ...
        Format_String node{AST_Node::no_range, "${a} + ${b}"};
        auto result = node.evaluate(ctx);
        REQUIRE(result->is<frst::String>());
    ...
    SECTION("Escaped placeholders are treated as literal text")
    {
        ...
        Format_String node{AST_Node::no_range, R"(\\${name} \${name})"};
        auto result = node.evaluate(ctx);
        REQUIRE(result->is<frst::String>());
    ...
    SECTION("Literal dollar signs remain literal")
    {
        mock::Mock_Symbol_Table syms;
        Evaluation_Context ctx{.symbols = syms};
        Format_String node{AST_Node::no_range, "price $5"};
    ...
    SECTION("$${name} yields a literal $ then a placeholder")
    {
        ...
        Format_String node{AST_Node::no_range, "$${name}"};
        auto result = node.evaluate(ctx);
        REQUIRE(result->is<frst::String>());
    ...
    SECTION("Repeated placeholder name is looked up each time")
    {
        ...
        Format_String node{AST_Node::no_range, "${x}${x}"};
        auto result = node.evaluate(ctx);
        REQUIRE(result->is<frst::String>());
    ...
    SECTION("Backslashes without placeholders are preserved")
    {
        mock::Mock_Symbol_Table syms;
        Evaluation_Context ctx{.symbols = syms};
        Format_String node{AST_Node::no_range, R"(\\)"};
    ...
    SECTION("Null values format as null")
    {
        ...
        Format_String node{AST_Node::no_range, "value: ${x}"};
        auto result = node.evaluate(ctx);
        REQUIRE(result->is<frst::String>());
    ...
    SECTION("Missing name lookup raises an unrecoverable error")
    {
        Symbol_Table syms;
        Evaluation_Context ctx{.symbols = syms};
        Format_String node{AST_Node::no_range, "${missing}"};
    ...
    {
        CHECK_THROWS(Format_String(AST_Node::no_range, "${}"));
        CHECK_THROWS(Format_String(AST_Node::no_range, "${1}"));
        CHECK_THROWS(Format_String(AST_Node::no_range, "${foo${bar}}"));
    }

    SECTION("symbol_sequence yields placeholder usages in order")
    {
        Format_String node{AST_Node::no_range, R"(a ${x} b \${y} ${z})"};
