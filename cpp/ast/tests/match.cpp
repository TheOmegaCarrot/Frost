...
TEST_CASE("Match: evaluates target exactly once")
{
    ...
    REQUIRE_CALL(*target, do_evaluate(_)).TIMES(1).RETURN(Value::create(42_f));
...
TEST_CASE("Match: empty arm list throws after evaluating target")
{
    ...
    auto target = mock::Mock_Expression::make();
    REQUIRE_CALL(*target, do_evaluate(_)).TIMES(1).RETURN(Value::create(1_f));
...
TEST_CASE("Match: first-match-wins short-circuits later arms")
{
    ...
    auto result2 = mock::Mock_Expression::make();
    REQUIRE_CALL(*result2, do_evaluate(_)).RETURN(Value::create("second"s));
...
TEST_CASE("Match: arms are tried in declaration order")
{
    ...
    auto pat2 = mock::Mock_Match_Pattern::make();
    auto pat3 = mock::Mock_Match_Pattern::make();
    REQUIRE_CALL(*pat1, do_try_match(_, _)).IN_SEQUENCE(seq).RETURN(false);
    REQUIRE_CALL(*pat2, do_try_match(_, _)).IN_SEQUENCE(seq).RETURN(false);
    REQUIRE_CALL(*pat3, do_try_match(_, _)).IN_SEQUENCE(seq).RETURN(false);
...
TEST_CASE("Match: guard runs only after its pattern matches")
{
    ...
    std::vector<Match::Arm> arms;
    arms.push_back(
        guarded_arm(std::move(pat), std::move(guard), std::move(result)));
    auto m = make_match(std::move(target), std::move(arms));
...
TEST_CASE("Match: non-truthy guard skips the arm")
{
    ...
    std::vector<Match::Arm> arms;
    arms.push_back(
        guarded_arm(std::move(pat1), std::move(guard1), std::move(result1)));
    arms.push_back(arm(std::move(pat2), std::move(result2)));
    auto m = make_match(std::move(target), std::move(arms));
...
TEST_CASE(
    "Match: pattern receives an arm context distinct from the outer table")
{
...
TEST_CASE(
    "Match: guard's eval context has both pattern bindings and outer failover")
{
...
TEST_CASE("Match: pattern bindings shadow outer names of the same spelling")
{
    ...
    auto result = mock::Mock_Expression::make();
    REQUIRE_CALL(*result, do_evaluate(_)).LR_RETURN(_1.symbols.lookup("x"));
...
TEST_CASE("Match: multiple arms each contribute their own outer usages")
{
    ...
    auto pat3 = mock::Mock_Match_Pattern::make();
    auto result3 =
        std::make_unique<Name_Lookup>(AST_Node::no_range, "outer_c"s);
...
TEST_CASE(
    "Match: discard binding produces no Definition and no spurious suppression")
{
    ...
    auto pat = std::make_unique<Match_Binding>(AST_Node::no_range, std::nullopt,
                                               std::nullopt);
